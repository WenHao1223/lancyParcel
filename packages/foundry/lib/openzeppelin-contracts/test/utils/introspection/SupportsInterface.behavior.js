	...
	describe("ERC165", function () {
		...
		describe("when the interfaceId is supported", function () {
			...
			it("returns true", async function () {
				for (const k of interfaces) {
					const interfaceId = INTERFACE_IDS[k] ?? k;
					expect(await this.contractUnderTest.supportsInterface(interfaceId), `does not support ${k}`).to.be
						.true;
				}
			});
		});
		...
		describe("when the interfaceId is not supported", function () {
			...
			it("returns false", async function () {
				expect(await this.contractUnderTest.supportsInterface(INVALID_ID), `supports ${INVALID_ID}`).to.be
					.false;
			});
		});
