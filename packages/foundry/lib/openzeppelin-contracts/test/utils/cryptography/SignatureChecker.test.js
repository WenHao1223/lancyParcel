	...
	describe("ERC1271 wallet", function () {
		for (const fn of ["isValidERC1271SignatureNow", "isValidSignatureNow"]) {
			describe(fn, function () {
				it("with matching signer and signature", async function () {
					expect(await this.mock.getFunction(`$${fn}`)(this.wallet, TEST_MESSAGE_HASH, this.signature)).to.be
						.true;
				});

				it("with invalid signer", async function () {
					expect(await this.mock.getFunction(`$${fn}`)(this.mock, TEST_MESSAGE_HASH, this.signature)).to.be
						.false;
				});

				it("with invalid signature", async function () {
					expect(await this.mock.getFunction(`$${fn}`)(this.wallet, WRONG_MESSAGE_HASH, this.signature)).to.be
						.false;
				});

				it("with malicious wallet", async function () {
					expect(await this.mock.getFunction(`$${fn}`)(this.malicious, TEST_MESSAGE_HASH, this.signature)).to
						.be.false;
				});
			});
		}
