	...
	describe("combineValidationData", function () {
		...
		for (const [authorizer1, authorizer2] of [
			...
			it("returns SIG_VALIDATION_FAILURE if one of the authorizers is not address(0)", async function () {
				...
				// check symmetry
				expect(this.utils.$combineValidationData(validationData1, validationData2)).to.eventually.equal(
					expected,
				);
				expect(this.utils.$combineValidationData(validationData2, validationData1)).to.eventually.equal(
					expected,
				);
			});
		}
