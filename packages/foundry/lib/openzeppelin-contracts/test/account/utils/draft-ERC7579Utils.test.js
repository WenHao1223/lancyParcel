	...
	describe("global", function () {
		...
		describe("eqModePayloadGlobal", function () {
			it("returns true if both payloads are equal", async function () {
				expect(this.utilsGlobal.$eqModePayloadGlobal(ethers.toBeHex(0, 22), ethers.toBeHex(0, 22))).to
					.eventually.be.true;
			});

			it("returns false if both payloads are different", async function () {
				expect(this.utilsGlobal.$eqModePayloadGlobal(ethers.toBeHex(0, 22), ethers.toBeHex(1, 22))).to
					.eventually.be.false;
			});
		});
	});
