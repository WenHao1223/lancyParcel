...
describe("ERC1363", function () {
	...
	describe("transferAndCall", function () {
		...
		it("reverts transferring to an EOA", async function () {
			await expect(
				this.token.connect(this.holder).getFunction("transferAndCall(address,uint256)")(this.other, value),
			)
	...
	describe("approveAndCall", function () {
		...
		it("reverts approving an EOA", async function () {
			await expect(
				this.token.connect(this.holder).getFunction("approveAndCall(address,uint256)")(this.other, value),
			)
		...
		it("succeeds without data", async function () {
			await expect(
				this.token.connect(this.holder).getFunction("approveAndCall(address,uint256)")(this.spender, value),
			)
